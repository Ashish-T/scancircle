...
export default function DashboardPage() {
  ...
  // Fixed Logout to redirect directly to Login page
  ...
  const renderSocialLinksTab = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6 bg-gradient-to-br from-white/[0.04] to-cyan-500/[0.01] p-6 rounded-3xl border border-cyan-500/20 backdrop-blur-2xl">
        <div>
          <label className="block text-xs font-mono text-cyan-400 uppercase tracking-widest mb-2">Business Designation</label>
          <input type="text" value={businessName} onChange={(e) => { setBusinessName(e.target.value); markAsUnsaved(); }} className="w-full px-4 py-3 bg-[#030307] border border-cyan-500/30 rounded-xl text-sm text-cyan-100 focus:border-cyan-400 outline-none font-mono" />
        </div>
        <div>
      ...
      <div className="bg-gradient-to-br from-white/[0.04] to-indigo-500/[0.01] p-8 rounded-3xl border border-indigo-500/20 backdrop-blur-2xl">
              ...
              <div key={dest.id} className="flex flex-col md:flex-row gap-6 items-start md:items-center p-6 bg-black/60 rounded-2xl border border-white/5 hover:border-cyan-500/40 transition-all">
                <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" value={dest.title} onChange={(e) => { setDestinations(destinations.map(d => d.id === dest.id ? { ...d, title: e.target.value } : d)); markAsUnsaved() }} className="w-full px-4 py-3 bg-[#030307] border border-white/10 rounded-xl text-sm text-gray-200 outline-none font-mono" />
                  </div>
                  <div>
                    ...
                    <input type={isUrl ? 'url' : 'text'} value={dest.value} onChange={(e) => { setDestinations(destinations.map(d => d.id === dest.id ? { ...d, value: e.target.value } : d)); markAsUnsaved() }} placeholder="Target vector..." className="w-full px-4 py-3 bg-[#030307] border border-white/10 rounded-xl text-sm text-gray-200 outline-none font-mono" />
                  </div>
                </div>
  ...
  const renderMenuBuilderTab = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-white/[0.04] to-amber-500/[0.01] p-8 rounded-3xl border border-amber-500/20 backdrop-blur-2xl">
        ...
        <div className="mb-8 p-6 bg-black/60 border border-white/5 rounded-2xl">
          ...
          <div className="flex gap-4 max-w-md">
            <input type="text" placeholder="New Category Vector..." value={newCategoryInput} onChange={(e) => setNewCategoryInput(e.target.value)} className="flex-1 px-4 py-3 bg-[#030307] border border-white/10 rounded-xl text-sm text-gray-200 outline-none font-mono" />
            <button onClick={() => { if(newCategoryInput && !menuCategories.includes(newCategoryInput)){ setMenuCategories([...menuCategories, newCategoryInput]); setActiveMenuCategory(newCategoryInput); setNewCategoryInput(''); markAsUnsaved(); } }} className="px-5 py-3 bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-xs rounded-xl hover:bg-amber-500/30">Create Category</button>
          </div>
        </div>
  ...
  const renderAnalyticsTab = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-white/[0.04] to-emerald-500/[0.01] p-8 rounded-3xl border border-emerald-500/20 backdrop-blur-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="p-6 bg-black/60 border border-emerald-500/20 rounded-2xl">
  ...
  const renderQRStudioTab = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-white/[0.04] to-cyan-500/[0.01] p-8 rounded-3xl border border-cyan-500/20 backdrop-blur-2xl flex flex-col items-center justify-center min-h-[450px]">
        ...
        <div className="w-72 bg-[#030307] rounded-3xl border-2 border-cyan-400/50 flex flex-col items-center p-6 mb-6 shadow-2xl relative">
        ...
        <button onClick={downloadBrandedQRCode} className="w-full py-4 px-6 bg-gradient-to-r from-cyan-400 to-indigo-500 text-black font-bold rounded-xl shadow-lg hover:opacity-90 font-mono tracking-wider">Download Holographic QR</button>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-4">
              <button onClick={() => { setQrColor('#22d3ee'); markAsUnsaved(); }} className="w-10 h-10 rounded-xl bg-cyan-400 ring-2 ring-offset-2 ring-offset-[#030307] ring-cyan-400"></button>
  ...
  const renderPlacesSyncTab = () => (
    <div className="bg-gradient-to-br from-white/[0.04] to-fuchsia-500/[0.01] p-8 rounded-3xl border border-fuchsia-500/20 backdrop-blur-2xl animate-in fade-in duration-500">
  ...
  const renderShopTab = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-white/[0.04] to-rose-500/[0.01] p-8 rounded-3xl border border-rose-500/20 backdrop-blur-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="p-6 bg-black/60 border border-white/5 rounded-2xl flex flex-col items-center justify-center">
            <div className="w-60 h-72 bg-[#030307] rounded-2xl border-2 border-rose-500/40 flex flex-col items-center p-4 shadow-xl mb-4">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleOrderTableStand} disabled={orderingStand} className="w-full py-4 px-6 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-bold rounded-xl shadow-lg hover:opacity-90 disabled:opacity-50 mt-2 font-mono">
      ...
      <div className="bg-gradient-to-br from-white/[0.04] to-cyan-500/[0.01] p-8 rounded-3xl border border-cyan-500/20 backdrop-blur-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="p-6 bg-black/60 border border-white/5 rounded-3xl text-center flex flex-col items-center">
            ...
            <div className="w-56 h-56 bg-white p-3 rounded-2xl shadow-xl flex items-center justify-center mb-4">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-black/60 p-6 rounded-2xl border border-white/5 space-y-4">
              ...
              <button onClick={handleSubmitUtr} disabled={submittingUtr} className="w-full py-3.5 px-6 bg-gradient-to-r from-cyan-400 to-indigo-500 text-black font-bold text-xs rounded-xl hover:opacity-90 shadow-lg disabled:opacity-50 font-mono tracking-wide">
  ...
  return (
    <div className="min-h-screen bg-[#030307] text-gray-200 flex flex-col md:flex-row relative overflow-hidden">
      ...
      <aside className="w-full md:w-72 bg-black/40 border-r border-cyan-500/10 flex flex-col backdrop-blur-2xl z-20">
        <div className="p-8 border-b border-cyan-500/10">
          {/* Logo link directs to Dashboard when logged in, or Login if not */}
          <Link href="/dashboard" className="flex items-center gap-2.5 mb-1 group focus:outline-none">
            <div className="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping"></div>
            <h2 className="text-xl font-extrabold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-400 font-mono group-hover:opacity-80 transition-opacity">SCAN_CIRCLE</h2>
          </Link>
        ...
        <nav className="p-6 space-y-2.5 flex-1 flex flex-col font-mono text-xs">
          <button onClick={() => setActiveTab('destinations')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'destinations' ? 'bg-cyan-500/15 text-cyan-300 border border-cyan-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// Social Links</button>
          <button onClick={() => setActiveTab('menu')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'menu' ? 'bg-amber-500/15 text-amber-300 border border-amber-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// Menu Matrix</button>
          <button onClick={() => setActiveTab('analytics')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'analytics' ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// Telemetry</button>
          <button onClick={() => setActiveTab('qr')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'qr' ? 'bg-indigo-500/15 text-indigo-300 border border-indigo-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// QR Studio</button>
          <button onClick={() => setActiveTab('places')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'places' ? 'bg-fuchsia-500/15 text-fuchsia-300 border border-fuchsia-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// Google Places</button>
          <button onClick={() => setActiveTab('shop')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'shop' ? 'bg-rose-500/15 text-rose-300 border border-rose-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// Hardware Store</button>
          
          <div className="mt-auto pt-4 border-t border-white/5">
            <button onClick={() => setActiveTab('subscription')} className={`w-full flex items-center px-5 py-3.5 rounded-xl transition-all ${activeTab === 'subscription' ? 'bg-cyan-500/15 text-cyan-300 border border-cyan-500/40 shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}>// Subscription & Billing</button>
          </div>
        </nav>
      </aside>
          ...
          <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-end gap-1">
                {saveStatus === 'saved' ? (
                  <button onClick={handleDeployConfig} className="py-3 px-8 bg-emerald-600/80 hover:bg-emerald-500 text-white text-xs font-mono font-bold rounded-xl shadow-lg flex items-center gap-2 border border-emerald-400/40 cursor-default">
                  ...
                  <button onClick={handleDeployConfig} disabled={saveStatus === 'saving'} className="py-3 px-8 bg-gradient-to-r from-cyan-400 to-indigo-500 text-black text-xs font-mono font-bold rounded-xl hover:opacity-90 shadow-lg disabled:opacity-50">
              ...
              <div className="relative">
                <button onClick={() => setProfileDropdownOpen(!profileDropdownOpen)} className="w-12 h-12 rounded-2xl bg-black/60 border border-cyan-500/30 flex items-center justify-center hover:border-cyan-400 transition-all shadow-lg">
                ...
                {profileDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-64 bg-[#030307] border border-cyan-500/30 rounded-2xl p-4 shadow-2xl z-30 font-mono">
