...
export default function RouterProfilePage({ params }: PageProps) {
  ...
  // Helper to log privacy-conscious tracking events
  const logEvent = async (eventType: string, targetBusinessId: string) => {
    try {
      ...
      await supabase.from('analytics_events').insert({
        ...
        device_type: device,
        browser: browser,
        country: 'IN', // Default context matching region
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      // Log landing view / QR scan event on load
  ...
  const handleLinkClick = (title: string) => {
    ...
    else logEvent('external_link_clicked', userId)
  }

  const handleReviewClick = () => {
    if (userId) logEvent('google_review_clicked', userId)
  }
